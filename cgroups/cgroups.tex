...
\markboth{Final}{Version \version{}}
...
Version 1.1 introduces the possibility of \emph{zero} subsystems `attached' to a hierarchy. The adjustments are
...
systasks' = systasks? \\
\exists CGroup' @ \\
\t1 ( ~~~ cg' = \{ ~ ROOTCGROUPNAME \mapsto \theta CGroup' ~\} \\
\t1 \land taskset' = systasks' ~~)
\end{schema}
...
to the same hierarchy\footnote{We originally thought the zero case was not possible (see section~\ref{sec:mailing list}).}.
...
It did not seem possible to define a \texttt{cgroup} hierarchy with no subsystems attached to it.
See the mailing list discussion\cite{noop}. We eventually discovered there is a special keyword that allows this!
This caused us to change our state model for $MultiSubsystems$, and in the process simplify some of our operations.
