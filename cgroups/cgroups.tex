...
Then we can consider how a new control group is created or destroyed.
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection*{System task operations}
...
and $DestroyTask$ when describing the effect upon an hierarchy. The create and destroy operation on
$Tasks$ can be elaborated to explain what happens on the extended state. 
This is called ``promotion'' and has well-trodden precedents in Z.

We first say what all of these elaborations have in common (expressed as a ``promotion'' schema), 
which in our case says that fundamental task operations only affect a single task in the hierarchy, while 
everything else remains fixed.

The $PromoteTaskOp$ says what remains fixed when promoting a system task operation to an hierarchy, with respect
to a single $task$. It is a skeleton operation schema.

\begin{schema}{PromoteTaskOp}
task : TASK
\also
\Delta CGHierarchy
\where
...
Except for a particular $task$, the system tasks are unaffected (neither created nor destroyed).
Except for this same $task$, the association between tasks and their control group path names is unaffected.
...
reverse all the (state) operations (swapping primed state for non-primed state---for 
example $cgroup'$ swaps with $cgroup$) and we have the same promotion schema.}
we can describe the next two operations easily.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection*{Create a new task in an hierarchy}

Here we promote $CreateTask$.

\begin{schema}{CreateTaskInCG}
CreateTask \\
PromoteTaskOp[task! / task]
\where
cgroup' ~ task! = cgroup ~ parent?
\end{schema}
The particular task is renamed to be the \emph{output} task (the one created). 
Everything else stays the same (see $PromoteTaskOp$) and the new $task!$ goes in the $parent?$'s control group.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection*{Destroy an existing task in an hierarchy}

Deleting a task from the system is even easier:
...
Everything else stays the same (see $PromoteTaskOp$). 
...
We are heartened by this description: $PromoteTaskOp$ captures exactly what we mean by a `task-local' change,
