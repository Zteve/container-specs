...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Initialisation}
We cannot say everything about the initial subsystem (until the $CGHierarchy$ is available), but we can
say:
\begin{zed}
InitSubsystem \defs [ Subsystem' | pinned' = \emptyset ]
\end{zed}
...
$Subsystem$ is pretty an\ae{}mic because it is not `attached' to a hierarchy. This is what we describe next.
...
\dom res = \dom cg
\end{schema}
Which says that the control group paths that the subsystem tracks are precisely the ones in
the hierarchy. The operations on $CGHierarchy$ and those on $Subsystem$ can now be lifted to $AttachedSubsystem$
by similar lifting mechanisms we used before.

What is interesting is that the constraints (aka preconditions) imposed by $CGHierarchy$ operations combine via the
$AttachedSubsystem$ constraint, to enable us to derive the preconditions of the lifted operations.
